...
public class VerticalDividerItemDecoration extends FlexibleDividerDecoration {
    ...
    protected void setItemOffsets(Rect outRect, int position, RecyclerView parent) {
        ...
        /**
         * GridLayoutManager extends LinearLayoutManager 特别注意这点
         */
        if (mShowFirstLastDivider  && (parent.getLayoutManager() instanceof GridLayoutManager) && position%spaceCount == 0){
                outRect.set(getDividerSize(position, parent), 0, getDividerSize(position, parent), 0);
        }else if (mShowFirstLastDivider && !(parent.getLayoutManager() instanceof GridLayoutManager)){
               outRect.set(getDividerSize(position, parent), 0, getDividerSize(position, parent), 0);
        }else {
    ...
    public static class Builder extends FlexibleDividerDecoration.Builder<Builder> {
        ...
        public Builder marginResId(@DimenRes int topMarginId, @DimenRes int bottomMarginId) {
            return margin(mResources.getDimensionPixelOffset(topMarginId),
                    mResources.getDimensionPixelOffset(bottomMarginId));
        }
