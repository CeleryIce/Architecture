package cc.ylike.corelibrary.RecyclerViewDivider;
...
public abstract class FlexibleDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    protected boolean mShowFirstLastDivider;
    protected int spaceCount = 1;
    ...
    protected FlexibleDividerDecoration(Builder builder) {
        ...
        mShowFirstLastDivider = builder.mShowFirstLastDivider;
        mPositionInsideItem = builder.mPositionInsideItem;
        spaceCount = builder.spaceCount;
    }
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < validChildCount; i++) {
            ...
            /**
             * LinearLayoutManager 单独处理
             */
            if (!hasDivider(parent, childPosition) && !(mShowFirstLastDivider && !(parent.getLayoutManager() instanceof GridLayoutManager))) {
                continue;
            }
            ...
            switch (mDividerType) {
                ...
                case PAINT:
                    ...
                    //横线绘制
                    if (mShowFirstLastDivider && childPosition == 0 && !(parent.getLayoutManager() instanceof GridLayoutManager)) {
                        c.drawLine(bounds.left, (float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), bounds.right, (float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), mPaint);
                    }else if (mShowFirstLastDivider && childPosition < spaceCount && parent.getLayoutManager() instanceof GridLayoutManager){
                        c.drawLine(bounds.left, (float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), bounds.right, (float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), mPaint);
                    }
                    //竖线绘制
                    if (mShowFirstLastDivider && childPosition%spaceCount == 0){
                        c.drawLine((float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), bounds.top, (float) (mSizeProvider.dividerSize(childPosition, parent)/2.0), bounds.bottom, mPaint);
                    }
    ...
    public boolean hasDivider(RecyclerView parent, int childPosition) {
        if (mShowLastDivider) {
            ...
            if ( mShowFirstLastDivider && (parent.getLayoutManager() instanceof GridLayoutManager) && childPosition%spaceCount == 0){
                return true;
            }
            return hasVerticalDivider(parent, childPosition);
        } else if (this instanceof HorizontalDividerItemDecoration) {
            if ((mShowFirstLastDivider && childPosition == 0)
                    || (mShowFirstLastDivider && childPosition < spaceCount && parent.getLayoutManager() instanceof GridLayoutManager)){
                return true;
            }
    ...
    public void getItemOffsets(Rect rect, View v, RecyclerView parent, RecyclerView.State state) {
        ...
        /**
         * 对 LinearLayoutManager 单独处理
         */
        if (!hasDivider(parent, position) && !(mShowFirstLastDivider && !(parent.getLayoutManager() instanceof GridLayoutManager))) {
            return;
        }
    ...
    public static class Builder<T extends Builder> {
        ...
        private boolean mShowFirstLastDivider = false;
        ...
        public T sizeResId(@DimenRes int sizeId) {
            return size(mResources.getDimensionPixelOffset(sizeId));
        }
        ...
        public T spaceResId(@DimenRes int spaceId) {
            return space(mResources.getDimensionPixelOffset(spaceId));
        }
        ...
        public T showGridFirstLastDivider() {
            spaceCount = 1;
            mShowFirstLastDivider = true;
            return (T) this;
        }
         ...
         * @return
         */
        public T showGridFirstLastDivider(int spaceCount) {
